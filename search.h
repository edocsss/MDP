...
const int normal_action_cost[3]{ 1, 1, 1 };
const int a_star_action_cost[3]{ 1, 12, 12 };
...
void update_knowledge(char * flat_map) {
	for (int i = 1; i <= N_rows; ++i) {
		memcpy(knowledge[i] + 1, flat_map + (i * N_cols - N_cols), N_cols);
	}
}
...
Sensor sensors[N_sensors]{ Sensor(-1, 1, 0, 2), Sensor(0, 1, 0, 2), Sensor(1, 1, 0, 2), Sensor(-1, 1, -1, 2), Sensor(-1, -1, -1, 2), Sensor(1, 1, 1, 5) };
...
struct BotState {
	...
	bool is_clear_left(const bool &ignore_unexplored = true) {
		bool clear_left = true;
		int _x = -bot_size - 1;
		for (int _y = -bot_size; _y <= bot_size && clear_left; ++_y) {
			pair<int, int> temp_coord = rotate(_x, _y, dir);

			int abs_x = pos_x + temp_coord.first,
				abs_y = pos_y + temp_coord.second;
			clear_left = clear_left && knowledge[abs_y][abs_x] != WALL && (ignore_unexplored || !ignore_unexplored && knowledge[abs_y][abs_x] != UNEXPLORED);
		}
		return clear_left;
	}
	...
	shared_ptr<list<tuple<shared_ptr<BotState>, Action, int>>> get_children(const bool &ignore_unexplored = true) {
		...
		for (int i = FORWARD; i <= TURN_LEFT; ++i) {
			...
			children->push_back(make_tuple(next_state, action, normal_action_cost[i]));
		}
...
struct A_StarState {
	...
	shared_ptr<list<tuple<shared_ptr<A_StarState>, Action, int>>> get_children() {
		...
		for (int i = FORWARD; i <= TURN_LEFT; ++i) {
			...
			children->push_back(make_tuple(next_state, action, a_star_action_cost[i]));
		}
